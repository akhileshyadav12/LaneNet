...
def _parse_function(filename, label_bin,label_ins,img_dir):
	...
	label_ins_resized = tf.cast(tf.image.resize_images(label_ins_decoded, [img_h, img_w],method='nearest'),dtype=tf.uint8)


	# label_img_instance = tf.zeros_like(label_ins_resized,dtype=tf.uint8)

	# lane_ins_color= tf.constant([50,50,50],dtype=tf.uint8)
	# ins_im1 = tf.cast(tf.reduce_all(tf.equal(label_ins_resized,lane_ins_color),axis=2),dtype=tf.uint8)*1
	# label_img_instance = tf.zeros_like(ins_im1,dtype=tf.uint8)
	# label_img_instance=tf.bitwise.bitwise_or(label_img_instance,ins_im1)

	# lane_ins_color= tf.constant([100,100,100],dtype=tf.uint8)
	# ins_im1 = tf.cast(tf.reduce_all(tf.equal(label_ins_resized,lane_ins_color),axis=2),dtype=tf.uint8)*2
	# label_img_instance=tf.bitwise.bitwise_or(label_img_instance,ins_im1)

	# lane_ins_color= tf.constant([150,150,150],dtype=tf.uint8)
	# ins_im1 = tf.cast(tf.reduce_all(tf.equal(label_ins_resized,lane_ins_color),axis=2),dtype=tf.uint8)*3
	# label_img_instance=tf.bitwise.bitwise_or(label_img_instance,ins_im1)

	# lane_ins_color= tf.constant([200,200,200],dtype=tf.uint8)
	# ins_im1 = tf.cast(tf.reduce_all(tf.equal(label_ins_resized,lane_ins_color),axis=2),dtype=tf.uint8)*4
	# label_img_instance=tf.bitwise.bitwise_or(label_img_instance,ins_im1)

	# lane_ins_color= tf.constant([250,250,250],dtype=tf.uint8)
	# ins_im1 = tf.cast(tf.reduce_all(tf.equal(label_ins_resized,lane_ins_color),axis=2),dtype=tf.uint8)*5
	# label_img_instance=tf.bitwise.bitwise_or(label_img_instance,ins_im1)

	# label_img_instance=tf.reshape(label_img_instance,tf.shape(label_ins_resized))
...
def image_data_read(file_names_inp,labels_inp,labels_ins_inp,img_dir_path,img_h,img_w):
	...
	labels_ins = np.array(labels_ins_inp).reshape(labels_inp.shape[0],)
