...
# to a fixed shape.
def _parse_function(filename, label,img_dir,coord):
	...
	image_string = tf.read_file(inp_file_name)
	...
	bg_img = tf.cast(tf.reduce_all(tf.equal(label_resized,bg_color),axis=2),dtype=tf.uint8)
 	gt_image=tf.to_float(tf.stack((road_im1,road_im2,road_im3,road_im4,bg_img),axis=2)) #,road_im2,road_im3,road_im4
	...
	return image_resized, gt_image,coord



def image_data_read(file_names_inp,labels_inp,img_dir_path,coords_vals,img_h,img_w):
	# A vector of filenames.

	file_names = np.array(file_names_inp).reshape(file_names_inp.shape[0],)
	# print "))))))))))))))))))))))))))))):::::",file_names.shape
	labels = np.array(labels_inp).reshape(labels_inp.shape[0],)
	...
	dataset = tf.data.Dataset.from_tensor_slices((file_names, labels,img_dir,coords_tf))
