...
def discriminative_loss_single(
    ...
    mu_diff = tf.subtract(mu_band_rep, mu_interleaved_rep)
    tf.debugging.check_numerics(mu_diff,message="mu_diff")
    intermediate_tensor = tf.reduce_sum(tf.abs(mu_diff), axis=1)
    zero_vector = tf.zeros(1, dtype=tf.float32)
    bool_mask = tf.not_equal(intermediate_tensor, zero_vector)
    mu_diff_bool = tf.boolean_mask(mu_diff, bool_mask)
    tf.debugging.check_numerics(mu_diff_bool,message="mu_diff_bool")
    mu_norm = tf.norm(mu_diff_bool, axis=1)
    mu_norm = tf.subtract(2. * delta_d, mu_norm)
    mu_norm = tf.clip_by_value(mu_norm, 0., mu_norm)
    mu_norm = tf.square(mu_norm)
    tf.debugging.check_numerics(mu_norm,message="MU_NORM")
    l_dist = tf.reduce_mean(mu_norm)

    l_reg = tf.reduce_mean(tf.norm(mu, axis=1))

    tf.debugging.check_numerics(l_dist,message="l_dist")

    l_var=tf.where(tf.is_nan(l_var), tf.ones_like(l_var)*100, l_var)
    l_dist=tf.where(tf.is_nan(l_dist), tf.ones_like(l_dist)*100, l_dist)
    param_scale = 1.
    l_var = param_var * l_var
    l_dist = param_dist * l_dist
    l_reg = param_reg * l_reg
